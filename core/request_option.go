...
func (r *RequestOptions) ToHeader() http.Header {
	...
	if r.RequestTimestamp != nil {
		header.Set("Request-Timestamp", fmt.Sprintf("%v", r.RequestTimestamp.Format(time.RFC3339)))
	}
	if r.RequestID != nil {
		header.Set("Request-ID", fmt.Sprintf("%v", *r.RequestID))
	}
	return header
}
...
// RequestTimestampOption implements the RequestOption interface.
type RequestTimestampOption struct {
	RequestTimestamp *time.Time
}

func (r *RequestTimestampOption) applyRequestOptions(opts *RequestOptions) {
	opts.RequestTimestamp = r.RequestTimestamp
}

// RequestIDOption implements the RequestOption interface.
type RequestIDOption struct {
	RequestID *string
}

func (r *RequestIDOption) applyRequestOptions(opts *RequestOptions) {
	opts.RequestID = r.RequestID
}
